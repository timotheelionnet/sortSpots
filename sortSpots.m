function [fname,nspots_per_ROI,ROI_vals,integrated_Int_per_ROI,ROI_volume] = batch_sort_spots_using_masks7()
...
if params.BatchMode == 1
    ...
    fname = assemble_multiple_file_lists2(listing,prefix,suffix);
    
    disp(['Found ',num2str(size(fname,1)),' files']);
else
    for i=1:nChannels
        fname{1,i} = locFileName{i};
    end
    fname{1,nChannels+1} = maskFileName;
...
for i=1:size(fname,1)
    %load mask image
    disp(['Treating File: ',fname{i,nChannels+1}]);
    mask = timtiffread(fname{i,nChannels+1});
    
    %merge all non backgound ROIs into one if option has been selected
    ROIlevels = unique(mask(:));
    ROI_vals{i,1} = ROIlevels;
    ...
    if strcmp(params.mode,'discard bg')
        ROI_vals{i,1} = ROI_vals{i,1}(ROI_vals{i,1} ~=min(ROI_vals{i,1}));    
    end  
    ...
    for j = 1:nChannels
        %load detected spots
        [spots,header] = readLocFile(fname{i,j});
        ...
        [~,fname_no_ext] = fileparts(fname{i,j});
        if isempty(header)
            fout = fullfile(out_dir,[fname_no_ext,'_mask_sorted.loc3']);
            save(fout,'spots2','-ascii');
        else
            fout = fullfile(out_dir,[fname_no_ext,'_mask_sorted.loc4']);
            header = [header,'RoiID'];
        ...
        ROI_vals{i,j} = ROI_vals{i,1}; %replicate mask ROI identities
        nROIs = numel(ROI_vals{i,j});

        NL = sprintf('\r\n');
        summary_filename = fullfile(out_dir,[fname_no_ext,'_mask.sum']);

        fid1 = fopen(summary_filename,'w');

        str = ['Detection file: ',fname{i,j},NL,...
            'Mask File: ', fname{i,nChannels+1},NL,];
        ...
        if isempty(spots)
            for k = 1:nROIs
                nspots_in_current_ROI = 0;
                integrated_Int_per_ROI{i,j}(k,1) = 0;
                nspots_per_ROI{i,j}(k,1) = 0;
                cur_ROI_volume = sum( mask(:) == ROI_vals{i,j}(k,1) );
                ROI_volume{i,j}(k,1) = cur_ROI_volume;
                str = ['ROI value: ',num2str(ROI_vals{i,j}(k,1)),'; ',...
                    num2str(nspots_in_current_ROI),...
                    ' spots detected; ROI volume: ',...
                    num2str(cur_ROI_volume),' voxels'];
                fprintf(fid1,'%s\r\n',str);
            end
        else
            for k = 1:nROIs
                nspots_in_current_ROI = ...
                    sum( spots2(:,nc+1) == ROI_vals{i,j}(k,1) );
                integrated_Int_per_ROI{i,j}(k,1) = ...
                    sum( spots2( spots2(:,nc+1) == ...
                    ROI_vals{i,j}(k,1),params.ndims+1 ) );
                nspots_per_ROI{i,j}(k,1) = nspots_in_current_ROI;
                cur_ROI_volume = sum( mask(:) == ROI_vals{i,j}(k,1) );
                ROI_volume{i,j}(k,1) = cur_ROI_volume;
...
%save global analysis to text file
save_filename = fullfile(out_dir,'spot_sort_stats.txt');
save_batch_sort_spots_results_in_txt(save_filename,fname,ROI_vals,...
    nspots_per_ROI,integrated_Int_per_ROI,ROI_volume);
